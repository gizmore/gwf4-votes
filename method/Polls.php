...
final class Votes_Polls extends GWF_Method 
{
	...
	{
		return
			'RewriteRule ^show_poll/(\d+)/[^/]+$ index.php?mo=Votes&me=Polls&show=$1 [QSA]'.PHP_EOL.
			'RewriteRule ^poll_overview$ index.php?mo=Votes&me=Polls [QSA]'.PHP_EOL.
			'RewriteRule ^poll_overview/by/([^/]+)/([DEASC,]+)/page-(\d+)$ index.php?mo=Votes&me=Polls&by=$1&dir=$2&page=$3 [QSA]'.PHP_EOL;
	}
