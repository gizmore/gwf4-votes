...
<tr><td colspan="4"><span id="vm_<?php echo $pid; ?>"><?php echo $reveal ? $total : '????'; ?></span>&nbsp;<?php echo $lang->lang('votes'); ?></td></tr>
<tr><td colspan="4"><input type="hidden" name="vmid" value="<?php echo $pid; ?>" /></td></tr>
<?php
$i = 1;
$lang_votes = '&nbsp;'.$lang->lang('voted');
foreach($o as $i => $opt)
{
	if ($reveal) {
...
?>
<tr><td colspan="4">
	<?php if ($may_edit) { echo GWF_Button::generic($lang->lang('btn_edit'), $tVars['href_edit']); } ?>
	<?php if ($may_vote) { ?>
		<input type="submit" name="vote" value="<?php echo $lang->lang('btn_vote'); ?>" />
	<?php } ?>
</td></tr>
</table>
</form>
